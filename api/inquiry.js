const RECIPIENT = "esmaniedwin@gmail.com";
...
async function sendEmail(payload) {
  ...
  if (!apiKey) {
    const error = new Error("Automatic email is not configured yet.");
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return res.status(400).json({ ok: false, error: "Invalid inquiry data." });
  }
  ...
  const inquiry = {
    ...
    commodity: clean(body.commodity, 300),
    origin: clean(body.origin, 160),
    mode: clean(body.mode, 80),
    summary: clean(body.summary, 4_000),
  ...
  const sender =
    ...
    "Eve Las Website <onboarding@resend.dev>";
  ...
  const rows = [
    ...
    ["Commodity / subject", inquiry.commodity],
